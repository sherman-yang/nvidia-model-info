...
This project provides a lightweight dashboard to explore the free AI models available at `build.nvidia.com`. It queries the model metadata endpoints, processes and flattens the result schemas, and presents them in an interactive, sortable, and filterable tabular view.
...
- **Live Ping & Limits Detection**: Click "Ping" on any model row to measure response latency and automatically detect context length and max output tokens via NVIDIA error response scraping. Results are cached to `model_limits_cache.json` and persist across restarts.
...
- **Sorting & Filtering**: Click headers to swap ascending/descending sort. Use the global search box to perform a sub-string filter across all parameters.
...
Full architectural documentation and requirement breakdowns are available in the repository root:
